...
router.get("/",async(req,res)=>{
    ...
    let month = req.query.month;
    if(month == null){
        const costReport = await CostsModel.find({ user_id: user_id}).find({year:year}).select("category").select("day").select("description").select("sum");
        res.send(costReport);

        }

    else{
        const costReport = await CostsModel.find({ user_id: user_id}).find({year:year}).find({month:month}).select("category").select("day").select("description").select("sum");
        res.json(costReport);
